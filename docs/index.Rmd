...
abstract: >
  ...
  For example, while Test group users were more likely to engage with results than the Control group on Arabic and Polish Wikipedias; counter-intuitively, the opposite was true on German and Italian Wikipedias. Given that we did observe a positive difference in engagement in 5 out of 8 wikis, Analysis' recommendation is to move forward with the cross-wiki search project.
  
  That is, there does not appear to be overwhelming evidence that we should *not* continue with the cross-wiki search project, and we are left to wonder if -- perhaps -- there are cultural differences in tasks/intent at play here, but this would require considerable human-computer interaction research.
output:
  html_document:
    includes:
