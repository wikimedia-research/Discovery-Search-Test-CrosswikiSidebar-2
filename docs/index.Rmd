...
table_caps(name = "Sampling Rates", caption = "Sampling rates used for event logging (EL) and random selection into the A/B test.")
...
figure_caps(name = "Clickthrough Rates (1)", caption = ifelse(is_html(), "Clickthrough rates of experimental groups, split by wiki. In the <span class=\"test-group-1\">Control</span> group, only searches that yielded some same-wiki results were considered. In the <span class=\"test-group-2\">Test</span> group, only searches that yielded some sister-wiki results were considered.", "Clickthrough rates of experimental groups, split by wiki. In the Control group, only searches that yielded some same-wiki results were considered. In the Test group, only searches that yielded some sister-wiki results were considered."))
figure_caps(name = "Clickthrough Rates (2)", caption = ifelse(is_html(), "Clickthrough rates of experimental groups, split by wiki. In the <span class=\"test-group-1\">Control</span> group, only searches that yielded some same-wiki results were considered. In the <span class=\"test-group-2\">Test</span> group...", "Clickthrough rates of experimental groups, split by wiki. In the Control group, only searches that yielded some same-wiki results were considered. In the Test group..."))
figure_caps(name = "BCDA H1", caption = ifelse(is_html(), "Comparison of the <span class=\"test-group-1\">Control</span> group with the <span class=\"test-group-2\">Test</span> group.", "Comparison of the Control group with the Test group."))
...
Users who made it into the test were then randomly assigned to one of the three groups described above: <span class="test-group-1">Control</span> and <span class="test-group-2">Test</span>.
...
See Phabricator ticket [T160004](https://phabricator.wikimedia.org/T160004) and Gerrit changes , and [343104](https://gerrit.wikimedia.org/r/#/c/343104) for full details of the implementation on both back-end and front-end.

## Analysis

We employed the *binom* [@R-binom], *LearnBayes* [@R-learnbayes], *conting* [@R-conting], and internally-developed *BCDA* [@R-BCDA] packages for Bayesian statistical analysis and confidence intervals in Figures `r figure_caps("zero_results_rate", display = "num")`, `r figure_caps("clickthrough_rates", display = "num")`, and `r figure_caps("BCDA H1", display = "num")`.
...
session_counts %>%
  ...
  set_colnames(c("Control", "Test", "All 2 groups")) %>%
...
click_counts %>%
  as.data.frame.matrix %>%
  set_rownames(c("Control", "Test", "All 2 groups")) %>%
...
searches_H1 <- searches %>%
  ...
  keep_where(
    (group == "Control" & `cirrus log: some same-wiki results`) |
      (group == "Test" & `cirrus log: some sister-wiki results`)
  ) %>%
  select(serp_id) %>%
...
searches_H2 <- searches %>%
  ...
  dplyr::do(binom::binom.bayes(.$clickthroughs, .$searches, conf.level = 0.95))
```
```{r hypotheses_captions, include=FALSE}
hypotheses_cap <- list(
  ctr_1 = format_caption(figure_caps, "Clickthrough Rates (1)"),
  ctr_2 = format_caption(figure_caps, "Clickthrough Rates (2)")
)
```
```{r hypothesis_1_plot, echo=FALSE, fig.cap=hypotheses_cap$ctr_1}
searches_H1 %>%
...
```
```{r hypothesis_2_plot, echo=FALSE, fig.cap=hypotheses_cap$ctr_2}
searches_H2 %>%
  ggplot(aes(x = 1, color = group, y = mean, ymin = lower, ymax = upper)) +
  geom_hline(aes(yintercept = mean), linetype = "dashed", color = RColorBrewer::brewer.pal(3, "Set1")[1],
             data = keep_where(searches_H2, group == "Control")) +
  geom_pointrange(position = position_dodge(width = 0.5)) +
  geom_text(aes(label = sprintf("%.1f%%", 100 * mean), y = upper + 0.0025, vjust = "bottom"),
            position = position_dodge(width = 0.5)) +
  facet_wrap(~ wiki, nrow = 1) +
  ...
  labs(x = NULL, title = "Clickthrough rates of experimental groups across wikis",
       subtitle = "Each point estimate includes a 95% credible interval",
       caption = "* For this engagement analysis we focused on searches which yielded both sets of results, even if the user did not necessarily see both.")
```

```{r bcda_H1_data}
bcda_H1 <- searches_H1 %>%
  keep_where(group %in% c("Control", "Test")) %>%
...
<!-- In Figures `r figure_caps("Clickthrough Rates (1)", display = "num")` and `r figure_caps("BCDA H1", display = "num")`, we see that the clickthrough rate was higher in <span class="test-group-2">Test</span> and <span class="test-group-3">Test (Recall)</span> than in <span class="test-group-1">Control</span> on almost all of the four wikis. The only exception being the clickthrough rates of users in the Polish Wikipedia <span class="test-group-2">Test</span> (`r sprintf("%.1f%%", 100 * keep_where(searches_H1, wiki == "Polish Wikipedia", group == "Test")$mean)`) and Italian <span class="test-group-3">Test (Recall)</span> (`r sprintf("%.1f%%", 100 * keep_where(searches_H1, wiki == "Italian Wikipedia", group == "Test (Recall)")$mean)`) groups.-->
